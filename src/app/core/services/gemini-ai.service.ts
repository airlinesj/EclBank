...
export class GeminiAiService {
  ...
  private buildSystemContext(): string {
    ...
    - Churn risk analysis and mitigation recommendations
    - Global financial data from World Bank, IMF, and other credited sources
    
    You provide data-driven insights with proper attribution to sources. When discussing ECL, rates, churn prediction, and market trends, reference credible sources and provide numerical analysis.`;
  }
  ...
  private generateFallbackResponse(userMessage: string): string {
    ...
    if (lowerMessage.includes('churn') || lowerMessage.includes('retention') || lowerMessage.includes('customer loss')) {
      return this.generateChurnResponse();
    }
...
6. **Churn Prediction** - Customer retention analysis, risk assessment, mitigation strategies
7. **Risk Assessment** - Default probability, loss potential, sector vulnerabilities
...
Please ask me about any specific financial topic, and I'll provide detailed data-driven analysis with proper source attribution. What would you like to explore?`;
  ...
  private generateChurnResponse(): string {
    return `**Customer Churn Analysis & Retention Intelligence:**

**Current Churn Status (Q4 2025):**
- Average churn risk score: 42.5/100
- Critical risk customers: 5% of portfolio
- High risk customers: 15% of portfolio
- Churn rate vs industry benchmark: 2.3x higher

**Key Churn Drivers:**
1. **Declining Engagement** (35% of customers)
   - Reduced account activity year-over-year
   - Lower product utilization trends

2. **Service Quality Issues** (22% above average)
   - Complaint rate increase: +15% QoQ
   - Resolution time degradation

3. **Competitive Pressure** (45% of high-risk segment)
   - Competitive offers received: +12% YoY
   - Market switching cost reduction

4. **Product Fit Misalignment** (28% affected)
   - Using only 28% of available products
   - Underutilized premium features

**Risk Distribution by Segment:**
- Premium Tier: 8% average churn risk
- Standard Tier: 45% average churn risk
- Basic Tier: 62% average churn risk

**Recommended Retention Actions:**
- **Critical Risk:** Immediate relationship manager outreach + VIP consultation
- **High Risk:** Personalized engagement campaigns + loyalty incentives
- **Medium Risk:** Regular touchpoints + feature education programs

**Projected Impact:**
- Potential churn reduction: 8-12% with interventions
- Customer lifetime value recovery: $2.3M annually
- ROI on retention investments: 3.5:1

Would you like detailed analysis for a specific customer segment or intervention strategy recommendations?`;
  }
